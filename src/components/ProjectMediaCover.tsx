...
import { Play, Volume2, Maximize2, ExternalLink } from 'lucide-react';
import { Project } from '../types';
import { extractYouTubeId, getYouTubeThumbnails, getYouTubePreviewEmbedUrl, getYouTubeFullEmbedUrl } from '../utils/youtube';
...
export const ProjectMediaCover: React.FC<ProjectMediaCoverProps> = ({
  ...
  const ytThumbs = videoId ? getYouTubeThumbnails(videoId) : null;

  // Decide thumbnail source: YouTube HQ/Maxres thumbnail if available, or project.bannerImage
  const thumbnailSrc = (!imgFailed && ytThumbs?.maxres) 
    ? ytThumbs.maxres 
    : (project.bannerImage || ytThumbs?.hq || '/assets/pvpprac1.0beta.png');

  // Handle hover debounce (start preview after 250ms of hovering, just like YouTube desktop)
  ...
  return (
    ...
    >
      ...
      {isPlayingFull && videoId ? (
        ...
        /* 3. Static Thumbnail Mode (YouTube Video Thumbnail or Banner) */
